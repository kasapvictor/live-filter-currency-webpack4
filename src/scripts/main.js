...
function choiceCurrency (e) {
	inputFilter.value = '';
	inputFilter.value = e.target.getAttribute('data-currency-name');
	hideFilterList();
}

function showFilterList () {
	filterList.innerHTML = "";
	switchClass(filterList, 'hide', 'show');
}

function hideFilterList () {
	switchClass(filterList, 'show', 'hide');
	filterClose.classList.add('hide');
	filterList.innerHTML = "";
}

function switchClass (elem, offClass, onClass) {
...
function searchValue (searchValue = '') {
	...
	for (const [key, value] of Object.entries(currencyList)) {
		for (let i in value ) {
			if ( value[i].toLowerCase().indexOf(searchValue) !== -1) {
				createLi(key, value['currencySymbol'], value['currencyName']);
				break
			}
		}
	}
...
function createLi (id, symbol = '&times;', name) {
	const li = document.createElement('li');
	li.setAttribute('data-currency-symbol', id);
	li.setAttribute('data-currency-name', name);
